...
// new
void QBase::tensor_product_quad(const QBase & q1Dx, const QBase & q1Dy)
{

  const unsigned int npx = q1Dx.n_points();
  const unsigned int npy = q1Dy.n_points();

  _points.resize(npx * npy);
   // std::cout << "1" << std::endl ;
  _weights.resize(npx * npy);
  ...
  for (unsigned int j=0; j<npy; j++)
    for (unsigned int i=0; i<npx; i++)
      {
        _points[q](0) = q1Dx.qp(i)(0);
        _points[q](1) = q1Dy.qp(j)(0);
       // std::cout << "2" << std::endl ;
        _weights[q] = q1Dx.w(i)*q1Dy.w(j);
