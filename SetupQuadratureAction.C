...
SetupQuadratureAction::validParams()
{
  ...
  // new
  params.addParam<std::vector<std::string>>("custom_orderX",
                                            std::vector<std::string>{},
                                            "list of customized quadrature orders (e.g. FIRST, SECOND, etc.)");
  params.addParam<std::vector<std::string>>("custom_orderY",
                                            std::vector<std::string>{},
                                            "list of customized quadrature orders (e.g. FIRST, SECOND, etc.)");
...
SetupQuadratureAction::SetupQuadratureAction(const InputParameters & parameters)
  : Action(parameters),
    ...
    //new
    _custom_orderX(getParam<std::vector<std::string>>("custom_orderX")),
    _custom_orderY(getParam<std::vector<std::string>>("custom_orderY"))
...
SetupQuadratureAction::act()
{
  ...
  */
    ...
    /// new
  // add custom block-specific quadrature rules with orders _custom_orderX and _custom_orderY
   
  for (unsigned int i = 0; i < _custom_blocks.size(); i++)
  {
    _problem->createQRules(_type,
                           _order,
                           Moose::stringToEnum<Order>(_custom_orderX[i]),
                           Moose::stringToEnum<Order>(_custom_orderX[i]),
                           _custom_blocks[i],
                           _allow_negative_qweights);
  }
  ...
  for (unsigned int i = 0; i < _custom_blocks.size(); i++)
  {
    _problem->createQRules(_type,
                           _order,
                           Moose::stringToEnum<Order>(_custom_orderY[i]),
                           Moose::stringToEnum<Order>(_custom_orderY[i]),
                           _custom_blocks[i],
                           _allow_negative_qweights);
  }
